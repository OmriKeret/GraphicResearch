...
public class OptitrackRigidBody : MonoBehaviour {	
	public int ID;
	public int scale = 1;
	private Vector3 planeModification;
    ...
    void Start() {
		...
		planeModification = new Vector3 (1, -1, 1);
    }
	...
	void Update () {
		...
		if(foundIndex == false) 
		{
            ...
            if (usePostionTracking)
                ...
                {
				transform.position = adjustPlane(((OptitrackRigidBodyManager.instance.rigidBodyPositions[index] - OptitrackRigidBodyManager.instance.origin.position) + originOverride.transform.position) * scale);
                }
                else
				transform.position = adjustPlane(OptitrackRigidBodyManager.instance.rigidBodyPositions[index] * scale) ;
	...
	public Vector3 adjustPlane(Vector3 origin) {
		Vector3 result = new Vector3 ();
		result.x = origin.x * planeModification.x;
		result.y = origin.y * planeModification.y;
		result.z = origin.z * planeModification.z;
		return result;
	}
